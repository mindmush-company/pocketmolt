...
export async function POST(
  ...
  try {
    ...
    if (bot.status !== "stopped") {
      return NextResponse.json(
        { error: `Cannot start bot in '${bot.status}' state` },
        { status: 400 }
      )
    }
    ...
    await hetzner.servers.powerOn(bot.hetzner_server_id)
    ...
    try {
      await hetzner.servers.waitForRunning(bot.hetzner_server_id, {
       ...
       console.warn("Timed out waiting for server to run, leaving as starting", e)
    }
